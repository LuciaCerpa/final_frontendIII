    ...
    <div className="card"> 
      <Tooltip text="Doctor Details">
        <Link to={`/detail/${dentist.id}`}>
          <img src="/images/doctor.jpg" alt={dentist.username} height={200} width={200}/>
          <h5>{dentist.username + dentist.id}</h5>
        </Link>
      </Tooltip>  
      <Tooltip text={findFav ? "Remove from favorites" : "Add to favorites"}>
        <button onClick={addFav} className="favButton"><img src={`/images/${findFav ? "star" : "darkStar"}.png`} height={25}/></button>
      </Tooltip>
    </div>
  );
};
